function [U,S,V,Out] = computSVDForLargeMatrix(A,r,opts)
%
...
[m,n] = check_matrix(A);
if r > min(m,n)/2;
    warning("TooManySVSrequested", min(m,n)/2);
end
% set parameters
mainargin = nargin;

[tol, maxit, idisp, memo, Y] = set_param;
% initialize
if isnumeric(A)
...
%% %%%%%%% nested functions %%%%%%% %%
    function [tol, maxit, idisp, memo, Y] = set_param
...
Out.hrvs = hrvs(1:iter,:);
    ...
    function [qtol, rtol, ptol] = set_tolerance
        tmp = min(mn/40/k,1);
        qtol = eps^tmp;
